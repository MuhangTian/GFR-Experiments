...
def cross_validation(model, param_dict: dict, disease: str, operations: dict, save_path: str=None, verbose: bool=False) -> Optional[dict]:
    ...
    for i in pbar:
        train = pd.read_csv(f"src/exp_6.6_to_6.27/data/mimic-disease/k-fold-id-no-cmo/TRAIN-{disease}-fold{i}.csv")
        test = pd.read_csv(f"src/exp_6.6_to_6.27/data/mimic-disease/k-fold-id-no-cmo/TEST-{disease}-fold{i}.csv")
...
if __name__ == "__main__":
    ...
    prs.add_argument("--algo", dest="algo", type=str, default='fasterrisk-17')
    ...
    if args.save_path == 'auto':
        args.save_path = f"src/exp_6.6_to_6.27/results/disease-no-cmo/{args.disease}/{args.algo}"
