...
def cross_validation(model, param_dict: dict, operations: dict, save_path: str=None, mode: str=None, verbose: bool=False) -> Optional[dict]:
    ...
    for i in pbar:
        train = pd.read_csv(f"src/exp_6.6_to_6.27/data/k-fold-no-cmo/TRAIN-union-features-fold{i}.csv")
        test = pd.read_csv(f"src/exp_6.6_to_6.27/data/k-fold-no-cmo/TEST-union-features-fold{i}.csv")
