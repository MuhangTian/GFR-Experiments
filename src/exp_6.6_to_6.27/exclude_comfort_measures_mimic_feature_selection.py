...
def train_ml_with_fasterrisk_features(model_name, mode, param_dict):
    ...
    for fold in pbar:
        train = pd.read_csv(f"src/exp_6.6_to_6.27/data/k-fold-no-cmo/TRAIN-union-features-fold{fold}.csv")
        test = pd.read_csv(f"src/exp_6.6_to_6.27/data/k-fold-no-cmo/TEST-union-features-fold{fold}.csv")
    ...
    os.makedirs(f"src/exp_6.6_to_6.27/results/compare-feature", exist_ok=True)
