...
def cross_validation(model, param_dict: dict, disease: str, operations: dict, save_path: str=None, verbose: bool=False) -> Optional[dict]:
    ...
    for i in pbar:
        ...
        X_train, y_train = train.drop(['hospital_expire_flag', 'subject_id', 'icustay_id', "hadm_id"], axis=1), train['hospital_expire_flag']
        X_test, y_test = test.drop(['hospital_expire_flag', 'subject_id', 'icustay_id', "hadm_id"], axis=1), test['hospital_expire_flag']
